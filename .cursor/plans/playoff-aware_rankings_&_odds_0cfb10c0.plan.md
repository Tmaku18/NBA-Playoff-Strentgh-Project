---
name: Playoff-Aware Rankings & Odds
overview: Add playoff data ingestion, playoff-performance ground truth, championship odds (softmax with temperature), conference-specific ranks, and new visuals/evaluations while preserving existing functionality via a config switch.
todos:
  - id: playoff-ingest
    content: Add playoff tables + raw ingestion (download + build DB).
    status: completed
  - id: playoff-rank
    content: Compute playoff-performance rank and helpers.
    status: completed
  - id: target-switch
    content: Add config switch + odds temperature.
    status: completed
  - id: inference-outputs
    content: Add global/conference ranks + odds + new visuals.
    status: completed
  - id: eval-docs
    content: Add playoff metrics + update README.
    status: completed
isProject: false
---
...
- Update **[src/data/db_schema.py](src/data/db_schema.py)** to add `playoff_games`, `playoff_team_game_logs`, `playoff_player_game_logs` (mirroring existing tables). Include `season` and `game_date` in `playoff_games` for easier joins.
- Extend **[src/data/db_loader.py](src/data/db_loader.py)**:
...
- Extend **[scripts/1_download_raw.py](scripts/1_download_raw.py)** to pull playoff logs for each season (2015–16 through 2025–26) using `nba_api` (SeasonType=`Playoffs`) and save to `data/raw/` with clear filenames.
- Extend **[scripts/2_build_db.py](scripts/2_build_db.py)** to ingest those new playoff files into the new tables.

### 2) Compute playoff-performance ground truth

Create a dedicated module, e.g. **[src/evaluation/playoffs.py](src/evaluation/playoffs.py)** (or `src/utils/playoffs.py`), to compute:
...
- Update **[scripts/4b_train_stacking.py](scripts/4b_train_stacking.py)** and **[src/training/train_stacking.py](src/training/train_stacking.py)** to use the playoff-performance rank as the training target when `training.target_rank == "playoffs"`.
- Keep all existing targets and outputs when the flag is `standings`.

### 4) Prediction outputs (JSON)

Update **[src/inference/predict.py](src/inference/predict.py)** to add new fields without removing existing ones:
...
Extend plotting in **[src/inference/predict.py](src/inference/predict.py)** or add a small viz helper module:
...
Update **[src/evaluation/metrics.py](src/evaluation/metrics.py)** and **[src/evaluation/evaluate.py](src/evaluation/evaluate.py)** to add:
...
- **Schema & loader:**
  - [src/data/db_schema.py](src/data/db_schema.py)
  - [src/data/db_loader.py](src/data/db_loader.py)
  - [scripts/1_download_raw.py](scripts/1_download_raw.py)
  - [scripts/2_build_db.py](scripts/2_build_db.py)
- **Playoff rank logic:**
  - [src/evaluation/playoffs.py](src/evaluation/playoffs.py) (new)
- **Training + inference:**
