...
todos:
  - id: sweep-cli-opts
    content: Add --batch-id/--val-frac and wire to outputs/split
    status: pending
  - id: run-epochs
    content: Run Model A epochs 8–28 step 1 with val_frac=0.25
    status: pending
  - id: run-model-b
    content: Run Model B phase-1 and phase-2 local grids
    status: pending
  - id: report-results
    content: Summarize best metrics and configs from the new sweeps
    status: pending
isProject: false
---
